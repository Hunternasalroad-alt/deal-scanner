...
4. **Sports category goes live.** All three sports resolve to category 261328 (§14.2). The scan loop iterates unique CATEGORIES (not games): 183454 implies pokemon; for 261328 the game is resolved PER LISTING from the Sport aspect (Baseball→baseball, Basketball→basketball, Football→football; missing or other → drop, dropReason `sport_unmapped`). Title keywords are never a game fallback — aspects-beat-titles (§6) holds.
5. **Matcher gate fixes.** (a) The sports find-or-create path must never insert a card with an empty name: an unparseable title drops with dropReason `unparseable_name`. (b) Two-letter name tokens (CJ, Bo, Ja) are legitimate player-name components and must survive sports name construction — the general len>2 usable-token filter may not erase them there. Both fail toward drop/LOW, never junk cards.
6. **Budget re-split.** Per-tick pages: 261328 up to 11, 183454 up to 7 (the measured 1.66:1 inflow ratio) within the unchanged INGEST_BUDGET_MS; MAX_PAGES_HARD 20 remains the absolute per-category cap. Ingest detail calls: 4 per category (was 8, one category). Daily arbitration stays with the existing machinery — 4,800 hard stop and the 3,600 sweep soft ceiling (built for exactly this cadence-tightening case).
