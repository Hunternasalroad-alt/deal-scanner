...
2. **Nightly prune.** In the nightly slot, after the re-score: delete dropped rows with firstSeen older than 7 days (their only job is deduping the recent search window), and null any `raw` still present on dropped rows (transition hygiene). Time-guarded like sibling phases; own-database work only.
