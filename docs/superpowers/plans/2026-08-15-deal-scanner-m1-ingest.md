...
- Produces: `syncPokemonPage(db: Db, page: PokeApiCard[]): Promise<{ upsertedCards: number; pricedCards: number }>` and `runPokemonSync(db: Db, fetchImpl?: typeof fetch): Promise<{ pages: number; upsertedCards: number }>` (pages through `https://api.pokemontcg.io/v2/cards?page=N&pageSize=250&select=id,name,number,set,tcgplayer`, header `X-Api-Key`). `PokeApiCard = { id: string; name: string; number?: string | null; set: { name: string; releaseDate?: string }; tcgplayer?: { prices?: Record<string, { market?: number | null }> } }` (inferred from a runtime-validated zod schema — see Step 4).
